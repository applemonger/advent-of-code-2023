...
fn read_data(path: &str) -> Vec<Race> {
    ...
    let times: Vec<i64> = time_regex.captures_iter(lines[0])
        .map(|c| {
    ...
    let dists: Vec<i64> = dist_regex.captures_iter(lines[1])
        .map(|c| {
    ...
    for (t, d) in times.iter().zip(dists.iter()) {
        races.push(Race { time: *t, distance: *d});
    }
    races
}
...
pub fn solution1() {
    ...
    let score: f64 = races.iter().map(|race| race.calculate_ways_to_win()).product();
    println!("{:?}", score);
}
