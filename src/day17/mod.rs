...
impl Grid {
    ...
    pub fn display(&self, path: &Vec<(i32, i32)>) {
        let mut loss: u64 = 0;
...
impl NodeStack {
    ...
    pub fn add_node(&mut self, node: Node, f_scores: &HashMap<(i32, i32), i32>) {
        ...
        self.nodes.sort_by(|a, b| {
            let a_score = *f_scores.get(&a.position).or_else(|| Some(&(i32::MAX / 2))).unwrap();
            let b_score = *f_scores.get(&b.position).or_else(|| Some(&(i32::MAX / 2))).unwrap();
            b_score.cmp(&a_score)
        });
    }
...
fn a_star(maze: &Grid, start: (i32, i32), end: (i32, i32)) -> Vec<(i32, i32)> {
    ...
    let mut open_set = NodeStack::new();
    ...
    while !open_set.is_empty() {
        ...
        'consideration: for neighbor in neighbors.iter() {
            let current_g_score = *g_scores.get(&current_node.position).or_else(|| Some(&(i32::MAX / 2))).unwrap();
            ...
            let neighbor_g_score = *g_scores.get(neighbor).or_else(|| Some(&(i32::MAX / 2))).unwrap();
            if tentative_g_score < neighbor_g_score {
                ...
                if !open_set.contains(*neighbor) {
                    open_set.add_node(Node { position: *neighbor }, &f_scores)
                }
            }
        }
    }
